...
This info was gathered from the RunCam Support in 06/2025:


To asses if Uboot is still accessible, the current of the Wifilink2 can be measured and compared to give values:
This can be achieved by a regulated power supply and/or a Multimeter.
Power will be applied via the 6pin FC connector

| Case  | Voltage | Amps | Power  | Status|
|:------|---------|------|--------|--------|
|1|13.5 V| 0.182 A| 2.5 W| Uboot still accessible <br>This suggests the firmware flash failed, but U-Boot is still present (device is not fully bricked).|
|2|13.5 V| <=0.144 A| <= 1.9 W| Uboot missing/unavailable <br>propably Bricked|
